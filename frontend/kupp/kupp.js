...
jQuery(function($) {
	...
	//Check if module list should be generated automatically (mittdesign)
	var myFrontPage = $("#mittdesign");

	//Check if there is a progress bar
	var myProgressbar = $("#minfremdriftsindikator");

	//Check if there is a custom module listing
	var myModule = $(".minmodul");


	if (myFrontPage.length || myProgressbar.length || myModule.length)
	{
		mmooc.api.getModulesForCurrentCourse(function(modules) {
			...
			//Check if we should fill out module links only.
			$(".minmodul").each(function( index ) {
				html = "<a href='" + modules[index].items[0].html_url + "'>" + modules[index].name + "</a>";
				$( this ).html(html) ;
			});
	...
	if(kuppModuleMenu.length)
	{
		mmooc.api.getCurrentModule(function(module) {
			...
			html += '<li class="module-home"><a href="/courses/' + courseId + '">Tilbake til moduloversikten</a></li>';
